...
- [ ] **Step 5b: fbc inside a model definition (ruling T11a).** libsbml 5.21.2 writes `fbc:strict` twice on a `<comp:modelDefinition>` and the file no longer parses, so the factory does not write it there. Measure over the corpus how many documents hold fbc content inside a model definition, what a PURE libsbml write/read copy of each does to `fbc:strict`, and what the round trip does. If the only difference which remains is the `fbc.strict` of a model definition, the structural comparison skips exactly that construct for a model definition, stated in its policy with the measurement and as narrow as the fbc v1 skip; it is not a whitelist entry. Decide with the numbers, and report them.
...
- [ ] **Step 4c: The port of a rule and of an initial assignment (ruling T13e).** libsbml's `getElementBySId` answers with neither a rule nor an initial assignment, so a port which names one by `idRef` has never validated (libsbml 1020702); a test of the previous task pins that error id. Such a port references its element by `metaIdRef` (the `_port_reference` mechanism of `Sbase` exists for this). Measure first which other element types the same holds for, failing test first, and flip the pinned error id to a document which validates.
- [ ] **Step 5: Verify and commit.** tox, lint, 148/148.
