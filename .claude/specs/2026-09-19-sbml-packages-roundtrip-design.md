...
**R5. Known, semantics-preserving normalizations are whitelisted explicitly in the structural comparison, each with a written reason.** They are: GPA same-operator flattening (above); `<cn>` gaining `type="integer"` (MathML round trips as an L3 infix string); an annotation resource becoming the compact `https://identifiers.org/<collection>:<term>` URL of the same collection and term (pymetadata canonicalization, idempotent). The entry is defined by its result, and the source is one of four forms: a `urn:miriam:` URN, a classic `http://identifiers.org/<collection>/<term>` URL, the `http://` spelling of the compact URL, or the bare compact identifier such as `UO:0000021` (widened from URNs only by rulings C1a and C1c of the ledger, after Task 1 measured pymetadata rewriting all four). A result which is not that compact URL is a loss and never whitelisted: a resource reduced to a bare term (`http://identifiers.org/sabiork/1406` to `1406`, `https://identifiers.org/CMO:0000012` to `CMO:0000012`) or stripped of its collection (`http://identifiers.org/slm/000000035` to `https://identifiers.org/000000035`). Nothing else may differ. *Cost if wrong: a real loss hidden behind a whitelist entry; each entry therefore needs a test showing the normalized form means the same thing.*
