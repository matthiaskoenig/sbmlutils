...
- **Generated ids and explicit units.** An assignment or rate rule without an id is written with the id `AssignmentRule_<variable>` or `RateRule_<variable>`, and a species without `substanceUnits` is written with the `substanceUnits` of the model, which it inherits anyway.
