...
- an element without math, which SBML allows from L3V2 on, round trips without math: a function definition, an initial assignment, a rule, a kinetic law, an event assignment, and the trigger, priority and delay of an event, as does an event without a trigger. The parser dropped each of them, an event whose trigger had no math with all its assignments, and logged an error for it. The value of these elements accepts `None`, and `Event(trigger=...)`, `priority` and `delay` take `None` for no element and an empty string for an element without math
