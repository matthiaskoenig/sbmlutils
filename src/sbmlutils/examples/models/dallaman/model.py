...
# Units
# ---------------------------------------------------------------------------------------------------------------------
model_units = ModelUnits(time=UNIT_min, extent=UNIT_KIND_MOLE, substance=UNIT_KIND_MOLE, length=UNIT_m,
                         area=UNIT_m2, volume=UNIT_m3)
units = [
    ...
    Unit('l_per_kg', [(UNIT_KIND_LITRE, 1.0),
                      (UNIT_KIND_KILOGRAM, -1.0)]),
    Unit('dl_per_kg', [(UNIT_KIND_LITRE, 1.0, -1, 1.0),
                       (UNIT_KIND_KILOGRAM, -1.0)]),
    Unit('mg_per_kg', [(UNIT_KIND_GRAM, 1.0, -3, 1.0),
                       (UNIT_KIND_KILOGRAM, -1.0)]),
    Unit('mg_per_dl', [(UNIT_KIND_GRAM, 1.0, -3, 1.0),
                       (UNIT_KIND_LITRE, -1.0, -1, 1.0)]),
    Unit('pmol_per_kg', [(UNIT_KIND_MOLE, 1.0, -9, 1.0),
                         (UNIT_KIND_KILOGRAM, -1.0)]),
    Unit('pmol_per_l', [(UNIT_KIND_MOLE, 1.0, -9, 1.0),
                        (UNIT_KIND_LITRE, -1.0)]),
    Unit('pmol_per_kgmin', [(UNIT_KIND_MOLE, 1.0, -9, 1.0),
                            (UNIT_KIND_KILOGRAM, -1.0), (UNIT_KIND_SECOND, -1.0, 0, 60)]),
    Unit('minkg_per_pmol', [(UNIT_KIND_SECOND, 1.0, 0, 60), (UNIT_KIND_KILOGRAM, 1.0),
                            (UNIT_KIND_MOLE, -1.0, -9, 1.0)]),
    Unit('mg_per_kgmin', [(UNIT_KIND_GRAM, 1.0, -3, 1.0),
                          (UNIT_KIND_KILOGRAM, -1.0), (UNIT_KIND_SECOND, -1.0, 0, 60)]),
    Unit('mgl_per_kgminpmol', [(UNIT_KIND_GRAM, 1.0, -3, 1.0), (UNIT_KIND_LITRE, 1.0),
                               (UNIT_KIND_KILOGRAM, -1.0), (UNIT_KIND_SECOND, -1.0, 0, 60),
                               (UNIT_KIND_MOLE, -1.0, -9, 1.0)]),
    Unit('mg_per_minpmol', [(UNIT_KIND_GRAM, 1.0, -3, 1.0),
                            (UNIT_KIND_SECOND, -1.0, 0, 60), (UNIT_KIND_MOLE, -1.0, -9, 1.0)]),

    Unit('pmolmg_per_kgdl', [(UNIT_KIND_MOLE, 1.0, -9, 1.0), (UNIT_KIND_GRAM, 1.0, -3, 1.0),
                             (UNIT_KIND_KILOGRAM, -1.0), (UNIT_KIND_LITRE, -1.0, -1, 1.0)]),
    Unit('pmolmg_per_kgmindl', [(UNIT_KIND_MOLE, 1.0, -9, 1.0), (UNIT_KIND_GRAM, 1.0, -3, 1.0),
                                (UNIT_KIND_KILOGRAM, -1.0), (UNIT_KIND_LITRE, -1.0, -1, 1.0),
...
parameters = [
    ...
    # Glucose Kinetics
    Parameter('V_G', 1.88, 'dl_per_kg', constant=True, name='V_G distribution volume glucose'),
    ...
    # Insulin kinetics
    Parameter('V_I', 0.05, 'l_per_kg', constant=True, name='V_I distribution volume insulin'),
    ...
    # Utilization
    Parameter('U_ii', 1, 'mg_per_kgmin', constant=True, name="insulin independent glucose utilization"),  # F_cns
...
rules = [
    ...
    AssignmentRule('EGP', 'k_p1-k_p2*Gp-k_p3*Id-k_p4*Ipo', 'mg_per_kgmin', name='EGP endogenous glucose production'),
    ...
    AssignmentRule('Ra', '1.32 dimensionless*f*k_abs*Qgut/BW', 'mg_per_kgmin', name='Ra glucose rate of appearance'),
    # % Ra', f*k_abs*Qgut/BW
    AssignmentRule('k_empt', 'k_min+(k_max-k_min)/2*(tanh(aa*(Q_sto-b*D))-tanh(cc*(Q_sto-d*D))+2)', '?'),
    AssignmentRule('U_idm', 'V_mmax*Gt/(K_m0+Gt)', 'mg_per_kgmin'),
    AssignmentRule('U_idf', 'V_fmax*Gt/(K_f0+Gt)', 'mg_per_kgmin'),
    AssignmentRule('U_id', 'U_idm+U_idf', 'mg_per_kgmin', name='insulin dependent glucose utilization'),
