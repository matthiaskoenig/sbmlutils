...
@timeit
def simulate_distrib_numpy(sbml_path: Path, n_samples: int):
    """Sample the initial values."""
    r: roadrunner.RoadRunner = roadrunner.RoadRunner(str(sbml_path))
    model: roadrunner.ExecutableModel = r.model
    r.selections = model.getGlobalParameterIds()
...
if __name__ == "__main__":
    ...
    n_samples = 5000
    # model with normal
    data_normal_roadrunner = simulate_distrib_roadrunner("model_normal.xml", n_samples=n_samples)
    data_normal_numpy = simulate_distrib_numpy("model_normal.xml", n_samples=n_samples)
    # model without distrib
    data_no_roadrunner = simulate_distrib_roadrunner("model_no_distrib.xml", n_samples=n_samples)
    ...
    ax1.set_title(f"roadrunner")
    ax2.set_title(f"numpy")
    plt.show()
    f.savefig(f"p_normal.png", bbox_inches="tight")
    # ---
    ...
    for kd, distribution in enumerate(r.selections):
        ...
        ax.set_xlabel("Value")
    plt.show()
    plt.savefig(f"distributions.png", bbox_inches="tight")
