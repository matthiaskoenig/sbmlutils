...
from sbmlutils.report import helpers
...
def test_unit_def1() -> None:
    ...
    assert helpers.unitDefinitionToString(unit_def) == "(mole)/(m^3)"


def test_unit_def2() -> None:
    unit = Unit("dimensionless", [(UNIT_KIND_DIMENSIONLESS, 1.0)])
    unit_def = unit.create_sbml(model)
    assert helpers.unitDefinitionToString(unit_def) == ""


def test_unit_def3() -> None:
    unit = Unit("pmol", [(UNIT_KIND_MOLE, 1.0, -12, 1.0)])
    unit_def = unit.create_sbml(model)
    assert helpers.unitDefinitionToString(unit_def) == "(10^-12)*mole"
